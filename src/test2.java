...
class test2 {
    ...
    public void initCategory(){
        ...
        for(int i =0; i < noOfCate; i++){
            try{
                ...
                cateIcon[i] = resizeImageIcon(cateIcon[i], cateIcon[i].getIconWidth() / 3, cateIcon[i].getIconHeight() / 3);
                ...
                if (itemPicFiles[i] != null) {
                    for (int j = 0; j < itemPicFiles[i].length; j++) {
                        if (itemPicFiles[i][j] != null) {
                            ...
                            itemPicIcons[i][j] = resizeImageIcon(itemPicIcons[i][j], itemPicIcons[i][j].getIconWidth() / 7, itemPicIcons[i][j].getIconHeight() / 7);
                        }
                    }
                } else {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(test2::new); // Ensure the UI updates on the Event Dispatch Thread
    }
}